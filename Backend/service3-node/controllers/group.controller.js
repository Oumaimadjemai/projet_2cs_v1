...
exports.createGroup = async (req, res) => {
  try {
    ...
    const moyenne = await groupService.calculateGroupAverage(
      group._id,
  ...
  try {
    ...
    if (!Array.isArray(members) || members.length === 0) {
      return res
        .status(400)
        .json({
          error: "Le tableau 'members' est requis et ne peut pas être vide.",
        });
    }
    ...
    try {
      const chefUserResponse = await axios.get(
        `${djangoUrl}/users/${chefId}/`,
        {
          headers: { Authorization: req.headers.authorization },
        }
      );
      ...
      console.warn(
        "Impossible de récupérer le nom complet via /users/:id, utilisation du nom partiel."
      );
...
exports.acceptInvitation = async (req, res) => {
  try {
    ...
    if (!userAnneeEtude) {
      return res
        .status(400)
        .json({ error: "Votre année d'étude n'est pas définie" });
    }
    ...
    const isComplete =
      group.members.length >= minMembres && group.members.length <= maxMembres;
...
exports.getChefThemeChoices = async (req, res) => {
  try {
    ...
    const groupsWithChefChoices = groups.map((group) => {
      const chefSelection = chefsSelections.find(
        (sel) => sel.user_id === group.chef_id
      );
...
exports.getGroupsByStudyYear = async (req, res) => {
  try {
    const requestedYear = req.query.annee_etude
      ? Number(req.query.annee_etude)
      : null;
    ...
    for (const group of allGroups) {
      try {
        ...
        const chef =
          membersDetails.find((m) => m.id === group.chef_id) ||
          membersDetails[0];
        const year =
          typeof chef.annee_etude === "number" ? chef.annee_etude : null;
...
exports.getGroupsByYearAndSpecialty = async (req, res) => {
  try {
    const requestedYear = req.query.annee_etude
      ? Number(req.query.annee_etude)
      : null;
    const requestedSpecialty = req.query.specialite
      ? Number(req.query.specialite)
      : null;
    ...
    for (const group of allGroups) {
      try {
        ...
        const year =
          typeof chef.annee_etude === "number" ? chef.annee_etude : null;
        const specialty =
          typeof chef.specialite === "number" ? chef.specialite : null;
  ...
  try {
    ...
    if (existingGroup) {
      return res.status(400).json({
        success: false,
        error:
          "L'utilisateur est déjà dans un autre groupe pour cette année académique",
        existing_group_id: existingGroup._id,
        existing_group_name: existingGroup.name,
...
exports.getUsersWithoutGroup = async (req, res) => {
  try {
    ...
    const etudiantsResponse = await axios.get(
      `${djangoUrl}/etudiants/annee/${annee_id}/`,
      {
        headers: { Authorization: req.headers.authorization },
        timeout: 5000,
      }
