...
function ThemesEnseignant() {
  ...
  useEffect(() => {
    axios
      .get(
        `http://127.0.0.1:8001/themes/enseignant/${localStorage.getItem(
          "user_id"
        )}/`
      )
      .then(async (res) => {
          ...
          themesData.map(async (theme) => {
            try {
              const anneeResponse = await axios.get(
                `http://127.0.0.1:8000/annees/${theme.annee_id}/`
              );
              ...
              console.error(
                `Erreur récupération année pour le thème ${theme.id}`,
                error
              );
              return {
                ...theme,
  ...
  const filteredThemes = themes.filter((theme) => {
    ...
    const isAnneeMatch =
      !selectedAnnee.id || theme.annee_id === selectedAnnee.id;

    // Filter by selected 'specialite_id' considering priority
    const isSpecialiteMatch =
      !selectedSpecialite ||
      theme.priorities.some(
        (priority) =>
          priority.specialite_id === selectedSpecialite &&
          priority.priorite === 1
      );
      ...
      <div className="themes-wrapper">
        <div className="btns-title-container">
          <div className="link">
            ...
            >
              Tous les Thèmes{" "}
              <span style={{ color: "#A7A7A7", marginLeft: "5px" }}>
                {themes.length}
              </span>
            </h1>
          </div>
          ...
          <div className="recherche-themes-input">
            <div className="filter-btn-div" style={{ position: "relative" }}>
              ...
              >
                ...
                >
                  <path
                    d="M7.742 6.05489L12.971 0.863892C13.1571 0.678984 13.4087 0.575195 13.671 0.575195C13.9333 0.575195 14.185 0.678984 14.371 0.863892C14.4627 0.954642 14.5354 1.06265 14.585 1.18169C14.6347 1.30072 14.6602 1.42842 14.6602 1.55739C14.6602 1.68636 14.6347 1.81406 14.585 1.93309C14.5354 2.05212 14.4627 2.16014 14.371 2.25089L8.44293 8.13589C8.25689 8.32079 8.00529 8.42458 7.74298 8.42458C7.48068 8.42458 7.22908 8.32079 7.04303 8.13589L1.11493 2.25089C1.02329 2.16014 0.950587 2.05212 0.90094 1.93309C0.851293 1.81406 0.825745 1.68636 0.825745 1.55739C0.825745 1.42842 0.851293 1.30072 0.90094 1.18169C0.950587 1.06265 1.02329 0.954642 1.11493 0.863892C1.3011 0.679226 1.55278 0.575607 1.815 0.575607C2.07723 0.575607 2.32878 0.679226 2.51495 0.863892L7.742 6.05489Z"
                    fill="#925FE2"
                  />
                </svg>
              </button>
              {filterSelected !== "none" && (
                <ul className="filtre-ul" ref={filterRef}>
                          ...
                          <li
                            ...
                            onClick={() =>
                              setSelectedAnnee({
                                ...
                                id: annee.id,
                                title: annee.title,
                                has_specialite: annee.has_specialite,
                              })
                            }
                          >
                            {annee.title}
                  ...
                  <li
                    id={
                      !(
                        selectedAnnee.title.includes("4") ||
                        selectedAnnee.title.includes("5")
                      )
                        ? "disabled-filter"
                        : ""
                    }
                  ...
                  >
                    Spécialité
                    <ArrowIcon />
                    {filterSelected === "specialite" &&
                      selectedAnnee.has_specialite && (
                        <ul className="sub-filter-ul">
                          {specialites.map((specialite) => (
                            <li
                              onClick={() =>
                                setSelectedSpecialite(specialite.id)
                              }
                            >
                              {specialite.title}
                            </li>
        ...
        {ajouterThemeClicked && <AjouterTheme annulerAjouter={cancelAjouter} />}
      </div>
    </div>
  );
}
...
const ThemeCard = ({ id, title, annee, specialite, valide, refus }) => {
  ...
  return (
    <div
      ...
      onClick={() => navigate(`enseignant/themes/${id}`)}
    >
      ...
      <div className="btns-card-line">
        ...
        {valide ? (
          <div className="accpeted-div">
            ...
            >
              <path
                d="M1.6665 10.5744L6.0415 14.741L8.229 12.241M6.6665 10.5744L11.0415 14.741L18.3332 6.40771M13.3332 6.40771L10.4165 9.74105"
                stroke="#5F5F5F"
                stroke-width="1.5"
          ...
          <div
            className="refused-div"
            style={{ backgroundColor: "#C4C4C4", color: "#5F5F5F" }}
          >
            <svg
              width="20"
              height="20"
