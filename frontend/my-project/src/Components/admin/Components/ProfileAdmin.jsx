...
function ProfileAdmin() {
    ...
    useEffect(() => {
        ...
        axios.get(`${process.env.REACT_APP_API_URL_SERVICE1}/admins/${userId}`)
            .then((res) => {
                setAdmin(res.data)
                setInitialAdminData(res.data)
            })
    ...
    const handleImageChange = (e) => {
        ...
        if (file) {
            setAdmin({ ...admin, photo_profil: file });
    ...
    useEffect(() => {
        if (initialAdminData.nom) {
            const photoChanged = imageSelected !== null;
            const dataChanged =
                admin.nom !== initialAdminData.nom ||
                admin.prenom !== initialAdminData.prenom;

            setIsModified(photoChanged || dataChanged);
        }
    }, [admin, imageSelected, initialAdminData]);
    ...
    const handleModifie = (e) => {
        ...
        if (admin.photo_profil instanceof File) {
            formData.append('photo_profil', admin.photo_profil);
        }
        if (admin.nom) formData.append('nom', admin.nom);
        if (admin.prenom) formData.append('prenom', admin.prenom);
        formData.append('email', admin.email)

        axios.put(`${process.env.REACT_APP_API_URL_SERVICE1}/admins/${admin.id}/`, formData, {
            headers: {
                ...
                <div className="photo-profile-line">
                    <div className="flex-line">
                        <div className="circle">
                            <img src={imageSelected || (admin.photo_profil ? admin.photo_profil : defaulImg)} alt="profile" />
                        </div>
