...
import '../../enseignante/Styles/Themes.css'
...
function ThemesEtudiant() {
    ...
    const [specialites, setSpecialites] = useState([])
      // Initialize selectedAnnee with proper default values
    const [selectedAnnee, setSelectedAnnee] = useState({
        id: null,
        title: "",
        has_specialite: false
    });
    const userAnneeEtude = localStorage.getItem('annee_etude');
    ...
    useEffect(() => {
        ...
        axios.get(`${process.env.REACT_APP_API_URL_SERVICE1}/annees/`)
            .then((res) => {
                setAnnees(res.data);
                // recuper annee etude de l'utilisateur
                if (userAnneeEtude) {
                    const userAnnee = res.data.find(a => a.id === parseInt(userAnneeEtude));
                    if (userAnnee) {
                        setSelectedAnnee({
                            id: userAnnee.id,
                            title: userAnnee.title,
                            has_specialite: userAnnee.has_specialite
                        });
                    }
                }
            })
    ...
    // useEffect(() => {
    //     axios
    //         .get(`${process.env.REACT_APP_API_URL_SERVICE2}/themes/by-annee/${selectedAnnee.id}/`)
    //         .then(async (res) => {
    ...
    useEffect(() => {
        const fetchUrl = selectedAnnee.id 
            ? `${process.env.REACT_APP_API_URL_SERVICE2}/themes/by-annee/${selectedAnnee.id}/`
            : `${process.env.REACT_APP_API_URL_SERVICE2}/themes/`;
                ...
                const themesWithDetails = await Promise.all(
                    themesData.map(async (theme) => {
                        ...
                        try {
                            ...
                            console.error(`Error fetching year for theme ${theme.id}`, error);
                        }
                        ...
                        try {
                            ...
                            console.error(`Error fetching speciality for theme ${theme.id}`, error);
                        }
    ...
    const [ajouterSuccess, setAjouterSuccess] = useState(false);
       ...
       const telechargerPdf = (id, nom) => {
        axios({
            url: `${process.env.REACT_APP_API_URL_SERVICE2}/themes/${id}/pdf/`,
            method: 'GET',
            responseType: 'blob',
            headers: {
                                ...
                                <table>
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Spécialité</th>
                                            <th style={{ width: "17.5%" }}>Nombre de Groupes</th>
                                            <th
                                                style={{
                                                    width: "100%",
                                                    ...
                                                    filteredThemes.map((theme) => (
                                                        <tr>
                                                            ...
                                                            >
                                                                ...
                                                                >
                                                                    <span style={{ display: "flex", flexDirection: "column", alignItems: "center", cursor: "pointer" }} className='download-btn' onClick={() => telechargerPdf(theme.id, theme.titre)}>
...

                    {
                        affichage === 'grid' && (
                            <div className="themes-cards-container">
                                {
                                    filteredThemes.map((theme) => (
                                        <ThemeCard id={theme.id} title={theme.titre} annee={theme.annee_titre} specialite={theme.specialite} />
                                    ))
                                }
                            </div>
                        )
...
export default ThemesEtudiant

const ThemeCard = ({ id, title, annee, specialite }) => {
    ...
    return (
        <div className="theme-card" onClick={() => navigate(`/etudiant/themes/${id}`)}>
